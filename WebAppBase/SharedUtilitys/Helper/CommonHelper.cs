...
namespace SharedUtilitys.Helper
{
    ...
    public class CommonHelper
    {
        ...
        public static bool ReaderContainField(IDataReader reader, string fieldName)
        {
            if (reader == null || string.IsNullOrWhiteSpace(fieldName))
            {
                return false;
            }
            for (int i = 0; i < reader.FieldCount; i++)
            {
                if (fieldName.Equals(reader.GetName(i)))
                {
                    return true;
                }
            }
